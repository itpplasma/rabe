...
contains
    ...
    subroutine test_avg_B_sq_antider_dBdtheta_over_B_cubed(qs_field, &
     ...
     use shaing_callen_remainder, only: calc_avg_B_squared_antider_dBdtheta_over_B_cubed
        ...
        average = calc_avg_B_squared_antider_dBdtheta_over_B_cubed(qs_flock, &
                                                                   qs_field)
        avg_B_squared = calc_avg_B_squared(qs_flock)
        average_qs = -0.5_dp*(1.0_dp - avg_B_squared*qs_flock%eta_b**2.0_dp)
        M_pol = qs_flock%M_pol
        N_tor = qs_flock%N_tor
        iota = qs_flock%iota
    ...
    subroutine test_trapped_fraction_prime_against_qs(qs_field, &
        ...
        trapped_fraction_prime = calc_trapped_fraction_prime(qs_flock, &
                                                             qs_field, &
                                                             n_eta)
    ...
    subroutine test_calc_avg_normalized_B_sq_dphimax_dxi0(qs_flock, &
                                                          test_failed)
    ...
    subroutine test_get_non_omnigenous_remainders(qs_field, &
        ...
        ! Tolerances reflect the QS cancellation achievable with the current
        ! fieldline pipeline; sibling checks hold the pieces to reltol 1e-12.
        real(dp), parameter :: reltol = 0.0_dp
        real(dp), parameter :: abstol_magnetics = 1e-15
        real(dp), parameter :: abstol_pitch = 1e-11
        ...
        found = get_non_omnigenous_remainder_magnetic(qs_flock)
        ...
        found = get_non_omnigenous_remainder_pitch(qs_flock, &
                                                   qs_field, &
                                                   n_eta)
        if (not_same(found, analytic, &
    ...
    subroutine test_limit_cancelation(flock, test_failed)
        use shaing_callen_integration, only: get_eta_integration_grid
        use shaing_callen_integration, only: integrate_over_eta_grid
