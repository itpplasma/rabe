...
vmec_file = os.path.join("output/wout_henneberg_qa.nc")
neo2_file = os.path.join("output/lambda_bb.h5")
bc_filename = os.path.join("output/quasi_axisymmetric.bc")
helicity_n = 0

with h5py.File(neo2_file, "r") as h5_file:
    s_tor = h5_file["boozer_s"][()][0]
    nu_star = h5_file["conl_over_mfp"][()]
