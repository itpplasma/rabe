...
contains
    ...
    !>
    !! \brief Total Shaing-Callen coefficient from the truncated general formula.
    !!
    !! \details The innermost field line integral is truncated to remove the
    !! bootstrap resonances, extending the omnigenous Landreman-Catto result by
    !! the non-omnigenous trapped fraction correction f_t'.
    !!
    !! \param[in] n_eta number of evaluation points to compute trapped fraction integral
    !! \param[in] dr_dAtheta converting lambda to be used with gradients of label r
    !<
    function calc_total_lambda_SC(flock, field, n_eta, dr_dAtheta) &
        result(total_lambda_SC)
        ...
        trapped_fraction_prime = calc_trapped_fraction_prime(flock, field, n_eta)
        call field%get_covariant_components(B_theta_cov, B_phi_cov)
        total_lambda_SC = -B_theta_cov*trapped_fraction + &
                          (B_phi_cov + flock%iota*B_theta_cov)*trapped_fraction_prime
        total_lambda_SC = total_lambda_SC*dr_dAtheta
    end function calc_total_lambda_SC

    function calc_trapped_fraction_prime(flock, &
                                         field, &
        ...
        avg_B_squared_over_avg_lambda = calc_avg_B_squared_over_avg_lambda(field, &
                                                                           flock, &
                                                                           eta_grid)
        F = calc_F_prime(flock, field, eta_grid)
        avg_B_squared_antider_dBdtheta_over_B_cubed = &
            calc_avg_B_squared_antider_dBdtheta_over_B_cubed(flock, field)
    ...
    function calc_F_prime(flock, field, eta_grid) result(F_prime)
        ...
        do this = 1, n_fieldlines
            F_prime = F_prime + calc_F_prime_for_fieldline(flock%fieldlines(this), &
                                                           eta_grid)
        end do
        F_prime = F_prime/sum(flock%fieldlines%integral_one_over_B_squared)
    ...
    function calc_avg_B_squared_antider_dBdtheta_over_B_cubed(flock, field) &
        result(res)
        ...
        allocate (this_field, source=field)
        res = 0.0_dp
        phi_grid = get_phi_integration_grid(flock%fieldlines(1))
        allocate (antider_dBdtheta_over_B_cubed(size(phi_grid)))
        do this = 1, n_fieldlines
            this_fieldline = flock%fieldlines(this)
    ...
    function get_non_omnigenous_remainder(flock, field, n_eta, dr_dAtheta) &
        ...
        remainder = get_non_omnigenous_remainder_pitch(flock, field, n_eta)
        remainder = remainder + get_non_omnigenous_remainder_magnetic(flock)
    ...
    function get_non_omnigenous_remainder_magnetic(flock) result(remainder)
        type(flock_of_fieldlines_t), intent(in) :: flock
        real(dp) :: remainder
        ...
        avg_B_squared = calc_avg_B_squared(flock)

        M_pol = flock%M_pol
        nfp = flock%nfp
        B_max = 1.0_dp/flock%eta_b
    ...
    function get_non_omnigenous_remainder_pitch(flock, field, n_eta) &
        result(remainder)
        use shaing_callen_mod, only: calc_avg_B_squared_over_avg_lambda
        type(flock_of_fieldlines_t), intent(in) :: flock
        class(field_t), intent(in) :: field
        ...
        integrand = calc_avg_B_squared_over_avg_lambda(field, flock, eta_grid)
    ...
    function calc_avg_B_squared(flock) result(avg_B_squared)
        type(flock_of_fieldlines_t), intent(in) :: flock
        real(dp) :: avg_B_squared

        avg_B_squared = sum(flock%fieldlines%phi_max(2) - flock%fieldlines%phi_max(1))
        avg_B_squared = avg_B_squared/sum(flock%fieldlines%integral_one_over_B_squared)
