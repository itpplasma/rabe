...
subroutine splinecof3_a(x, y, c1, cn, lambda1, indx, sw1, sw2, &
  ...
  !! Modifications by Andreas F. Martitsch (06.08.2014)
    ...
    use solve_systems, only: solve_eqsys
  ...
  !! End Modifications by Andreas F. Martitsch (06.08.2014)
    ...
    integer(I4B) :: size_dimension, maxpos_A(2), maxpos(1)
    integer(I4B) :: i_alloc, info, unit
    ...
    allocate (inh(size_dimension), indx_lu(size_dimension), stat=i_alloc, &
              errmsg=error_message)
    if (i_alloc /= 0) then
        write (*, *) 'splinecof3: Allocation for arrays inh and indx_lu '// &
    ...
    if (i_alloc /= 0) then
        write (*, *) 'splinecof3: Allocation for array omega failed '// &
            ' with message:'
        write (*, *) trim(error_message)
        write (*, *) 'size should be ', size(lambda1)
        stop
...
! ---------------------------
    ...
    ! solve system
    open (newunit=unit, file='matrix_vector.dat', status='replace', &
          action='write')
    write (unit, *) MA
    write (unit, *) inh
    close (unit)
...
subroutine splinecof3_lo_driv_a(x, y, c1, cn, lambda, w, indx, &
    ...
    if (i_alloc /= 0) stop 'splinecof3_lo_driv: '// &
        'Deallocation for arrays 1 failed!'
    deallocate (indx1, lambda1, hi, stat=i_alloc)
    if (i_alloc /= 0) stop 'splinecof3_lo_driv: '// &
        'Deallocation for arrays 2 failed!'
    deallocate (xn, yn, stat=i_alloc)
    if (i_alloc /= 0) stop 'splinecof3_lo_driv: '// &
        'Deallocation for arrays 3 failed!'
...
subroutine splinecof1_a(x, y, c1, cn, lambda1, indx, sw1, sw2, &
    ...
    do i = 1, len_indx - 1
        ...
        a(i) = y(i) ! - b(i) * x(i) ! this term cancels, because we assume
        ! coordinate system is centered at x(i), and thus x(i) = 0.
    end do
...
subroutine splinecof1_lo_driv_a(x, y, c1, cn, lambda, w, indx, &
    ...
    deallocate (ai, bi, ci, di, stat=i_alloc)
    if (i_alloc /= 0) stop 'splinecof1_lo_driv: '// &
        'Deallocation for arrays 1 failed!'
    deallocate (indx1, lambda1, hi, stat=i_alloc)
    if (i_alloc /= 0) stop 'splinecof1_lo_driv: '// &
        'Deallocation for arrays 2 failed!'
